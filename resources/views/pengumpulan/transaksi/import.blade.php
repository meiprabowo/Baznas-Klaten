@section('title',$title)
@extends('layout.app')
@section('content')
<div class="container-fluid">
                <div class="row">
                    <div class="col-lg-12">
                        <div class="shop-breadcrumb">

                            <div class="breadcrumb-main">
                                <h4 class="text-capitalize breadcrumb-title">Upload Data Transaksi</h4>
                                <div class="breadcrumb-action justify-content-center flex-wrap">
                                    <nav aria-label="breadcrumb">
                                        <ol class="breadcrumb">
                                            <li class="breadcrumb-item"><a href="#"><i class="las la-home"></i>Transaksi</a></li>
                                            <li class="breadcrumb-item active" aria-current="page">Upload Data</li>
                                        </ol>
                                    </nav>
                                </div>
                            </div>

                        </div>
                    </div>
                </div>
            </div>
            @if ($errors->any())
    <div class="alert alert-danger">
        <ul>
            @foreach ($errors->all() as $error)
                <li>{{ $error }}</li>
            @endforeach
        </ul>
    </div>
@endif

@if (session()->has('success'))
    <div class="alert alert-success dark" role="alert">
        <p>{{ session('success') }}</p>
    </div>
@endif

@if (session()->has('warning'))
    <div class="alert alert-danger dark" role="alert">
        <p>{{ session('warning') }}</p>
    </div>
@endif
@if (session()->has('failures'))
    @php
        $failureCount = count(session('failures'));
    @endphp
    <div class="alert alert-warning dark" role="alert">
        <p>
            Berhasil diUpload: {{ session('gagal') }} Data<br/>
            Gagal diUpload: {{ $failureCount }} Data ==> 
            <a href="#" onclick="downloadTableAsExcel()"><b> DOWNLOAD DATA GAGAL</b></a> <==
        </p>
    </div>
    <script>
        function downloadTableAsExcel() {
            var table = document.getElementById('downloaded');
            var html = table.outerHTML;

            // Convert HTML to Blob
            var blob = new Blob([html], { type: 'application/vnd.ms-excel' });

            // Create download link and trigger click
            var a = document.createElement('a');
            a.href = window.URL.createObjectURL(blob);
            a.download = 'failed_data.xls';
            document.body.appendChild(a);
            a.click();

            // Remove the temporary link
            document.body.removeChild(a);
        }
    </script>
    <table border="1" id="downloaded" style="display: none;">
        <tr> 
            <th>Baris</th>
            <th>Atribut</th>
            <th>Error</th>
            <th>Value</th>
        </tr>
        @foreach (session()->get('failures') as $validasi)
    <tr>
        <td>{{ $validasi->row() }}</td>
        <td>{{ $validasi->attribute() }}</td>
        <td>
            @foreach ($validasi->errors() as $error)
                <li>{{ $error }}</li>
            @endforeach
        </td>
        <td>
            {{-- Ambil nilai yang gagal dari baris data --}}
            {{ $row[$validasi->attribute()] ?? 'Tidak ada nilai' }}
        </td>
    </tr>
@endforeach

    </table>
@endif

<div class="col-12">
                  <div class="bg-white p-25 mb-25 radius-xl">
                     <div class="row">
                        <div class="col-lg-8 col-md-6">
                           <figure class="feature-cards7 feature-cards7--1">
                              <div class="banner-card banner-card-border">
                                 <div class="banner-card__top align-center-v justify-content-between">
                                 <h5 class="banner-card__title">
                                    Form Import Data Transaksi
                                    </h5>
                                 </div>
                                 <div class="banner-card__body">
                                    <p>
                                        
                                    <form id="uploadForm" method="POST" action="{{ route('pengumpulan.pengumpulan.importstore') }}" enctype="multipart/form-data">
                                    @csrf
                                    Upload Data Transaksi <small class="display-block">Migrasi harus menggunakan format yang telah ditentukan yaitu dalam bentuk xls / xlsx.                               <span><br/>
                                    <code class="text-danger"> Download template <a href="{{ route('pengumpulan.pengumpulan.download') }}" target="_blank"> ==> <b>DOWNLOAD</b> <== </a></code>
    </small>
                                    </p>
                                 </div>
                                 <div class="banner-card__bottom  align-center-v justify-content-between">
                                    
                                    <div class="col-md-6 mb-3" style="padding-right: 15px;">
                                        <label class="form-label" for="validationDefault02">Sumber Penyaluran</label>
                                        <select name="sumber" id="select-search" class="form-control" required>    
                                            <option value=""> -- Pilih Sumber --</option>
                                            @foreach($sumber as $key => $d)
                                            <option value="{{ $d->id }}">{{ $d->uraian }}</option>
                                            @endforeach
                                        </select>
                                    </div> 

                                    <div class="col-md-6 mb-3" style="padding-left: 15px;">
                                        <label class="form-label" for="validationDefault02">UPZ</label>
                                        <select name="dinas" id="dinas" class="form-control" required>      
                                            <option value="0"> -- Pilih Dinas --</option>
                                            @foreach($dinas as $key => $dd)
                                            <option value="{{ $dd->id }}">{{ $dd->nama_muzaki }}</option>
                                            @endforeach
                                        </select>
                                    </div>

                                                                
                                    
                                     <div class="col-md-8 mb-3">
                                           <label class="form-label" for="validationDefault02">Upload File</label>
                                           <input class="form-control" type="file" name="file" required>
                                      </div>
                                      <div class="col-md-8 mb-3">
                                      <button type="button" class="btn btn-outline-lighten btn-outline-lighten__height" data-bs-toggle="modal" data-bs-target="#modal-colored-danger">Simpan</button>
                           </div>                                 </div>
                           
     



<div class="modal-colored-danger modal fade show" id="modal-colored-danger" tabindex="-1" role="dialog" aria-hidden="true">


<div class="modal-dialog modal-md" role="document">
   <div class="modal-content modal-bg-danger modal-colored">
      <div class="modal-header">



         <h6 class="modal-title">Perhatian ..... !!!</h6>
        
      </div>
      <div class="modal-body">
      <p>Pastikan Tanggal , Jumlah dan Tujuan UPZ sudah benar ?</p>
      </div>
      <div class="modal-footer">
      <button type="button" class="btn btn-primary btn-outlined btn-sm" data-bs-dismiss="modal">Cek Lagi</button>
         <button type="submit" class="btn btn-info btn-sm" data-bs-dismiss="modal">Ya</button>
      </div>
   </div>
</div>








</div>      </form>
                              </div>
                           </figure>
                        </div>
                        
                        
                     </div>
                  </div>
               </div>

                </div>
      </div>
   </div>
</div>

<script type="text/javascript"> 
 $(document).ready(function() {
 $('#dinas').select2();
 });
</script>


@endsection



